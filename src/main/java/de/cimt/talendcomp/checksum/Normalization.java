...
public class Normalization {
    ...
    /**
     * Tracks if all input values are null, regardless if
     * {@link NormalizeConfig#nullReplacement nullReplacement} is set.
     */
    private boolean allInputsAreNull = true;

    /**
     * Tracks if any input value isn't null. It takes care of
     * {@link NormalizeConfig#nullReplacement nullReplacement}. If
     * nullReplacment is not null and only one null value is added, than
     * hashBaseHasValues is true!
     */
     ...
     */
    public String calculateHash(HashFunction type, HashOutputEncoding hashOutputEncoding) throws IllegalArgumentException {
        ...
        return hashOutputEncoding.encode( 
            type.digest( getNormalizedString() )
        );
    }
    /**
     * Calculates hash value based on added objects
...
//            return normalize((Byte) object);
    ...
    public String normalize(final String value, final NormalizeObjectConfig itemConfig) {
        ...
        if (value == null) {
            ...
            normalized=itemConfig.getCaseSensitive().process(normalized);
